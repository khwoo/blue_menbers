var vm = new Vue({
	...
	created:function(){
		...
		that.eventImageURL = `image/event04.png?v=${Date.parse(new Date())}`;
	},
	mounted: function() {
		var that = this;
		...
		that.linkCoordsList = [{
			coords:that.setRectPosition( 22.85 , 3 , 3.8 , 1 )
		}];
	...
	methods: {
		...
		,link:function( data ){
			var brandCd = '';
			try {
				location.href = `event_detail03.html?custNo=${this.key_custNo}&uid=${this.key_uid}`;
			}catch(e){
