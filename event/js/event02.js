var vm = new Vue({
	...
	created:function(){
		...
		that.eventImageURL = `image/event03.png?v=${Date.parse(new Date())}`;
	},
	mounted: function() {
		var that = this;
		...
		that.linkCoordsList = [{
			coords:that.setRectPosition( 17.33 , 3.1 , 3.8 , 1 )
		}];
	...
	methods: {
		...
		,link:function( data ){
			var brandCd = '';
			try {
				location.href = `event_detail02.html?custNo=${this.key_custNo}&uid=${this.key_uid}`;
			}catch(e){
